/*
 ...
 The scope of my tests are mainly concerned with ensuring the correctness of each function
 (constructor, overload, method) that I implemented. To do so I created brief tests for each
 ...
 Furthermore, I only need to ensure that each constructor works, and then ensure each
 operator/method works for just one type of construction. This is because once an object is
 successfully constructed, it will be treated the same by the operators/methods regardless of how it
 was constructed.

 Before anything else, I want to ensure that my code handles all edge cases appropriately. This
 includes things such as multiplying two vectors with different number of dimensions, calling the
 move assignment on a const EV, finding the norm of a zero vector, creating an empty vector or
 exception handling etc. However, I will not test some already discussed edge cases such as
 multiplying two 0 vectors, as it has been stated in the specs that that case will not be tested.

 To ensure correctness of my program, I wanted to test each constructor, operator, and method.
 For each one, I want to test it with both a const and non const Euclidean vector, to ensure that
 ...
 efficiently test other features).

 Overall, my tests should be quite thorough and cover quite a large scope/range of scenarios. By
 testing each and every individual constructor, overload, and method I implemented, as well as their
 different combinations of interactions, I should be able to cover most use cases and scenarios.
 Though the examples used may not be overly complex (There will be few EVs that are excessively
 long), that should be fine because the implementation for everything is quite simple, and extending
 the length of the EVs will just make longer iterations, which shouldn't ever produce unexpected
 behaviour.
...
SCENARIO("Checking that the default constructor works using the default value 1 when not "
         "specifying a size") {
...
SCENARIO("Checking that the copy constructor works") {
  GIVEN("An already existing Euclidean Vector Object") {
    ...
    WHEN("You try to copy it to another Euclidean Vector") {
      ...
      THEN("You should successfully construct a EV with the same values as the original and "
...
SCENARIO("Using subscript with an EuclideanVector") {
  GIVEN("A Euclidean Vector of 3 with magnitudes (1, 2, 3)") {
    ...
    WHEN("You try to use the subscript operator to store the value of each element in the array in "
         "a double") {
      double e1 = ev[0];
      double e2 = ev[1];
      double e3 = ev[2];
...
SCENARIO("Checking that the << operator works") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    ...
    WHEN("You try to use the << operator to print out the EV") {
      std::strstream s;
      s << ev;
      THEN("The output stream should print the contents of the EV surrounded by square brackets "
           "ie. [1 2 3]") {
...
SCENARIO("Using the copy assignment on an Euclidean Vector") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    ...
    WHEN("You try to use the copy assignment on it") {
      ...
      THEN("You should produce a copy of the original vector, and the original vector should still "
           "exist in the same state it was before the copy assignment") {
        REQUIRE(copy[0] == 1);
        REQUIRE(copy[1] == 2);
        REQUIRE(copy[2] == 3);
        REQUIRE(ev1[0] == 1);
...
SCENARIO("Using the move assignment on an Euclidean Vector") {
  GIVEN("A Euclidean vector {1,2,3}") {
    ...
    WHEN("You try to use the move assignment on it") {
      ...
      THEN("You should produce a copy of the original vector, and the original vector should no "
           "longer exist") {
        REQUIRE(move[0] == 1);
        REQUIRE(move[1] == 2);
        REQUIRE(move[2] == 3);
...
SCENARIO("Using the vector addition operator on two Euclidean Vectors") {
  GIVEN("A Euclidean vector {1,2,3 and another Euclidean Vector {2,3,4}") {
    ...
    WHEN("You try to use the vector addition operator on them to add their corresponding "
         "magnitudes") {
      EuclideanVector sum = ev1 + ev2;
      THEN("You should produce an Euclidean Vector with magnitudes equal to the corresponding sums "
           "of the magnitudes of the original two Euclidean Vectors") {
        REQUIRE(sum[0] == 3);
        REQUIRE(sum[1] == 5);
        REQUIRE(sum[2] == 7);
...
SCENARIO("Using the vector subtraction operator on two Euclidean Vectors") {
  GIVEN("A Euclidean vector {1,2,3 and another Euclidean Vector {2,3,4}") {
    ...
    EuclideanVector ev1{v1.begin(), v1.end()};
    EuclideanVector ev2{v2.begin(), v2.end()};
    WHEN("You try to use the vector subtraction operator on them to subtract their corresponding "
         "magnitudes") {
      EuclideanVector diff = ev1 - ev2;
      THEN("You should produce an Euclidean Vector with magnitudes equal to the corresponding "
           "differences of the magnitudes of the original two Euclidean Vectors") {
        REQUIRE(diff[0] == -1);
        REQUIRE(diff[1] == -1);
        REQUIRE(diff[2] == -1);
...
SCENARIO("Using Vector type conversion on an EuclideanVector") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    ...
    WHEN("You try to cast it to a vector") {
      ...
      THEN(
          "The resulting vector should have all the same values as the original Euclidean Vector") {
        REQUIRE(vect[0] == 1);
        REQUIRE(vect[1] == 2);
        REQUIRE(vect[2] == 3);
        REQUIRE(vect.size() == 3);
...
SCENARIO(
    "Using the GetNumDimensions method to find the number of dimensions in an Euclidean Vector") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v = {1, 2, 3};
    EuclideanVector ev{v.begin(), v.end()};
...
SCENARIO("Using the GetEuclideanNorm method to find the Euclidean norm of an Euclidean Vector") {
  GIVEN("A Euclidean Vector {2, 2, 2, 2}") {
    ...
    WHEN("You try to use the GetEuclideanNorm method to find the Euclidean Norm of the EV") {
      ...
      THEN("The return value of the function should equal the square root of the sum of the "
           "squares of the magnitudes in each dimension") {
        REQUIRE(norm == 4);
      }
    }
  }
...
SCENARIO("Using the CreateUnitVector method to create the unit vector of an Euclidean Vector") {
  GIVEN("A Euclidean Vector {2, 2, 2, 2}") {
    ...
    WHEN("You try to use the CreateUnitVector method to create the unit vector of the EV") {
      ...
      THEN("The magnitude for each dimension in the unit vector should equal the original vector's "
...
SCENARIO("Using == to test equality of two identical EuclideanVectors") {
  GIVEN("A Euclidean Vector {1,2,3} and another Euclidean Vector {1,2,3}") {
    ...
    WHEN("You try to use the == operator to see if they are identical") {
      bool same = v1 == v2;
      THEN("The result should return true") { REQUIRE(same == true); }
    }
  }
}
...
SCENARIO("Using == to test equality of two different EuclideanVectors") {
  GIVEN("A Euclidean Vector {1,2,3} and another Euclidean Vector {2,3,4}") {
    ...
    WHEN("You try to use the == operator to see if they are not identical") {
      bool same = v1 == v3;
      THEN("The result should return false") { REQUIRE(same == false); }
    }
  }
}
...
SCENARIO("Using the addition operator (+) on two different EuclideanVectors") {
  GIVEN("A Euclidean Vector {1,2,3} and another Euclidean Vector {2,3,4}") {
    ...
    WHEN("You try to use the addition operator to add their magnitudes together") {
      EuclideanVector sum = ev1 + ev2;
      THEN("The values of the sum EV should equal the sum of the respective magnitudes of the "
           "original 2 EVs") {
        REQUIRE(sum[0] == 3);
        REQUIRE(sum[1] == 5);
        REQUIRE(sum[2] == 7);
...
SCENARIO("Using the subtraction operator (-) on two different EuclideanVectors") {
  GIVEN("A Euclidean Vector {1,2,3} and another Euclidean Vector {2,3,4}") {
    ...
    WHEN("You try to use the subtraction operator to subtract their magnitudes together") {
      EuclideanVector sum = ev1 - ev2;
      THEN("The values of the sum EV should equal the difference of the respective magnitudes of "
           "the original 2 EVs") {
        REQUIRE(sum[0] == -1);
        REQUIRE(sum[1] == -1);
        REQUIRE(sum[2] == -1);
...
SCENARIO(
    "Using the scalar multiplication operator (*) on an EuclideanVector with the scalar in front") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v1 = {1, 2, 3};
    EuclideanVector ev1{v1.begin(), v1.end()};
...
SCENARIO("Using the scalar multiplication operator (*) on an EuclideanVector with the scalar in "
         "the back") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v1 = {1, 2, 3};
    EuclideanVector ev1{v1.begin(), v1.end()};
...
SCENARIO("Checking that using the + operator on 2 EVs with different number of dimensions throws "
...
SCENARIO("Checking that using the at method at an index greater than the number of dimensions in "
         "the EV throws the correct exception") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v1 = {1, 2, 3};
    EuclideanVector ev1{v1.begin(), v1.end()};
...
SCENARIO("Trying to use GetEuclideanNorm method on a vector with 0 dimensions") {
  GIVEN("A Euclidean Vector with 0 dimensions") {
    ...
    WHEN("I try to use the GetEuclideanNorm method on the 0 vector") {
      THEN("I should catch an exception") {
        REQUIRE_THROWS_WITH(test.GetEuclideanNorm(),
...
SCENARIO("Trying to use CreateUnitVector method on a vector with 0 Dimensions/EuclideanNorm") {
  GIVEN("A Euclidean Vector with 0 dimensions/EuclideanNorm") {
    ...
    WHEN("I try to use the CreateUnitVector method on the 0 vector") {
      THEN("I should catch an exception") {
        REQUIRE_THROWS_WITH(test.CreateUnitVector(),
...
SCENARIO("Trying to use * (dot product) friend operator on two EVs with a different number of "
         "dimensions") {
  GIVEN("Two EVs with a different number of dimensions") {
    std::vector<double> v1 = {1, 2, 3};
    std::vector<double> v2 = {1, 2};
...
// REPEATING ALL PREVIOUS TESTS BUT WITH A CONST EV INSTEAD (where appropriate, I wouldn't test
// const on features that need to change the calling EV)
...
SCENARIO("Using the move assignment on a const Euclidean Vector") {
  GIVEN("A Euclidean vector {1,2,3}") {
    ...
    WHEN("You try to use the move assignment on it") {
      EuclideanVector move = std::move(ev1);
      THEN("You should end up using the copy assignment instead (so the original will still "
           "exist)") {
        REQUIRE(move[0] == 1);
        REQUIRE(move[1] == 2);
        REQUIRE(move[2] == 3);
...
SCENARIO("Using the GetNumDimensions method to find the number of dimensions in a const Euclidean "
         "Vector") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v = {1, 2, 3};
    const EuclideanVector ev{v.begin(), v.end()};
...
SCENARIO(
    "Using the GetEuclideanNorm method to find the Euclidean norm of a const Euclidean Vector") {
  GIVEN("A Euclidean Vector {2, 2, 2, 2}") {
    std::vector<double> v = {2, 2, 2, 2};
    const EuclideanVector ev{v.begin(), v.end()};
    WHEN("You try to use the GetEuclideanNorm method to find the Euclidean Norm of the EV") {
...
SCENARIO(
    "Using the CreateUnitVector method to create the unit vector of a const Euclidean Vector") {
  GIVEN("A Euclidean Vector {2,2,2,2}") {
    std::vector<double> v = {2, 2, 2, 2};
    const EuclideanVector ev{v.begin(), v.end()};
    WHEN("You try to use the CreateUnitVector method to create the unit vector of the EV") {
...
SCENARIO("Using the scalar multiplication operator (*) on a const EuclideanVector with the scalar "
         "in front") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v1 = {1, 2, 3};
    const EuclideanVector ev1{v1.begin(), v1.end()};
...
SCENARIO("Checking that using the + operator on 2 const EVs with different number of dimensions "
...
SCENARIO("Checking that using the at method at an index < 0 on a const EV throws the correct "
         "exception") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v1 = {1, 2, 3};
    const EuclideanVector ev1{v1.begin(), v1.end()};
...
SCENARIO("Checking that using the at method at an index greater than the number of dimensions in "
         "the const EV throws the correct exception") {
  GIVEN("A Euclidean Vector {1,2,3}") {
    std::vector<double> v1 = {1, 2, 3};
    const EuclideanVector ev1{v1.begin(), v1.end()};
    WHEN("I try to use the at method at an index > the number of dimensions in the EV") {
...
SCENARIO("Trying to use CreateUnitVector method on a const Euclidean vector with 0 "
         "Dimensions/EuclideanNorm") {
  GIVEN("A Euclidean Vector with 0 dimensions/EuclideanNorm") {
    const EuclideanVector test(0);
    WHEN("I try to use the CreateUnitVector method on the 0 vector") {
      THEN("I should catch an exception") {
...
SCENARIO("Trying to use * (dot product) friend operator on two const EVs with a different number "
         "of dimensions") {
  GIVEN("Two EVs with a different number of dimensions") {
    std::vector<double> v1 = {1, 2, 3};
    std::vector<double> v2 = {1, 2};
