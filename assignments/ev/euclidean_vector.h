...
class EuclideanVector {
 public:
    ...
    friend EuclideanVector operator+(const EuclideanVector& v1, const EuclideanVector& v2) {
        if (v1.GetNumDimensions() != v2.GetNumDimensions()) throw EuclideanVectorError("Dimensions of LHS(X) and RHS(Y) do not match");
    ...
    // * operator to find the dot product of 2 EVs. Throws exception if the two EVs have different dimensions
    ...
    // division operator to divide each dimension of an EV by a scalar. Throws exception if trying to divide by 0
    ...
    double at(const int& n) const {
        if (n < 0 || n >= dimensions_) throw EuclideanVectorError("Index X is not valid for this EuclideanVector object");
        return magnitudes_[n];
    }

    // at method to set the value at a certain index in an EV. Throws exception if the index is out of bounds
    ...
    // method to get the Euclidean Norm of an EV. Throws exception if the number of dimensions in the EV is 0
    double GetEuclideanNorm() const;

    // method to create a unit vector from an EV. Throws exception if the EV has no dimensions or has a euclidean normal of 0
